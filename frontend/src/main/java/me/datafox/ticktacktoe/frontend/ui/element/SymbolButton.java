...
public class SymbolButton extends Button {
    ...
    public void draw(Batch batch, float parentAlpha) {
        ...
        center.add(offset);
        if(symbol.equals(Symbols.X)) DrawingUtils.drawX(Game.ui().getDrawer(), center, Math.min(offset.x, offset.y) - Defaults.WORLD_BASE_WIDTH * 2, 0, color.get(), false);
        else if(symbol.equals(Symbols.O)) DrawingUtils.drawO(Game.ui().getDrawer(), center, Math.min(offset.x, offset.y) - Defaults.WORLD_BASE_WIDTH * 2, color.get());
    }
